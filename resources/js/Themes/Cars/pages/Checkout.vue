    ...
    <div class="max-w-5xl mx-auto px-4 py-10">
      ...
      <div class="flex items-center gap-0 mb-8 mt-2">
        <div class="flex items-center gap-2">
          <div class="w-7 h-7 rounded-full bg-slate-100 text-slate-700 flex items-center justify-center text-xs font-bold">1</div>
          <span class="text-sm text-slate-700 font-semibold">{{ t('checkout.step_cart') }}</span>
        </div>
        <div class="flex-1 h-px bg-slate-200 mx-3" />
        <div class="flex items-center gap-2">
          <div class="w-7 h-7 rounded-full bg-slate-800 text-white flex items-center justify-center text-xs font-bold">2</div>
          <span class="text-sm text-slate-800 font-semibold">{{ t('checkout.step_details') }}</span>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
              ...
              <div class="space-y-3">
                ...
                >
                  <input type="radio" :value="method.key" v-model="form.payment_method" class="mt-0.5 text-slate-700" />
                  <div>
              ...
              <div v-if="form.payment_method === 'bank_transfer' && bankAccounts.length" class="mt-5 space-y-3">
                ...
                <div v-for="bank in bankAccounts" :key="bank.bank" class="bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm">
          ...
          <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sticky top-6">
              <h2 class="font-bold text-gray-900 mb-4">{{ t('checkout.rental_summary') }}</h2>
              ...
              <div class="space-y-3 mb-4">
                <div v-for="item in cart.items" :key="item.id" class="flex gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-slate-600">{{ item.quantity }} {{ t('booking.days') }}</div>
                    <div class="text-xs text-gray-500">{{ formatPrice(item.product.price) }} {{ t('activity.per_day') }}</div>
                  </div>
                </div>
              </div>
              ...
              <div v-if="pointsDiscount > 0" class="flex justify-between text-slate-700 text-sm">
              ...
              <div v-if="pointsBalance > 0" class="mt-4 p-3 bg-slate-50 rounded-xl border border-slate-200">
                <div class="flex items-center justify-between mb-2">
                  <span class="text-sm font-medium text-slate-900">{{ t('checkout.use_points') }}</span>
                  <span class="text-xs text-slate-600">{{ pointsBalance }} {{ t('checkout.points_available') }}</span>
                </div>
                <div class="flex items-center gap-2">
                  <input type="number" v-model.number="form.use_points" min="0" :max="maxRedeemable" step="100"
                    class="w-24 border border-slate-300 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" />
              ...
              <button type="submit" :disabled="submitting || !form.payment_method"
                class="w-full mt-5 bg-slate-800 text-white py-3 rounded-xl font-semibold hover:bg-slate-700 transition-colors disabled:opacity-60 cursor-pointer">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div v-for="item in upsells" :key="item.id"
            ...
            <div class="p-3 flex flex-col flex-1">
              ...
              <p class="text-slate-600 font-semibold text-sm mb-3">{{ formatPrice(item.price) }}</p>
              <a :href="route('product.show', item.slug)"
                class="mt-auto w-full py-1.5 text-xs font-semibold rounded-lg bg-slate-50 text-slate-700 hover:bg-slate-100 transition-colors text-center">
