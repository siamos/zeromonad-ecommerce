<template>
  <Layout>
    ...
    <div class="max-w-5xl mx-auto px-4 py-10">
      ...
      <p class="text-gray-500 mb-10">{{ t('blog.subtitle_cars') }}</p>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <article v-for="post in posts.data" :key="post.id"
          ...
          <img v-if="post.featured_image" :src="post.featured_image" :alt="post.title" class="w-full h-48 object-cover" />
          <div class="p-5">
            <div class="flex items-center gap-2 mb-3">
              <span v-if="post.ai_generated"
                class="text-xs bg-slate-100 text-slate-700 px-2 py-0.5 rounded-full font-medium">
                {{ t('blog.ai_generated') }}
              </span>
              <span class="text-xs text-gray-400">{{ post.published_at }}</span>
            </div>
            <h2 class="font-semibold text-gray-900 mb-2 line-clamp-2">
              <Link :href="route('blog.show', post.slug)" class="hover:text-slate-700 transition-colors">
                {{ post.title }}
              </Link>
            </h2>
            <p class="text-sm text-gray-500 line-clamp-3">{{ post.excerpt }}</p>
            <Link :href="route('blog.show', post.slug)"
              class="mt-4 inline-block text-sm text-slate-700 font-medium hover:underline">
              {{ t('blog.read_more') }}
            </Link>
          </div>
        </article>
