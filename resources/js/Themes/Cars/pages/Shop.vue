      ...
      <div class="flex flex-col md:flex-row gap-8">
        ...
        <aside class="w-full md:w-64 shrink-0">
          <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
            ...
            <div>
              ...
              <div class="space-y-1">
                <label class="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="radio" v-model="filters.category" value="" class="text-slate-700" />
                ...
                <label v-for="cat in categories" :key="cat.id" class="flex items-center gap-2 text-sm cursor-pointer">
                  <input type="radio" v-model="filters.category" :value="cat.slug" class="text-slate-700" />
            ...
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">{{ t('booking.pickup_date') }}</label>
              <input
                type="date"
            ...
            <button
              @click="applyFilters"
              class="w-full bg-slate-800 text-white py-2 rounded-lg text-sm font-semibold hover:bg-slate-700 transition-colors cursor-pointer"
            >
        ...
        <div class="flex-1">
          <div class="flex items-center justify-between mb-6">
            ...
            <select
              ...
              @change="applyFilters"
              class="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
            >
          ...
          <div v-if="hasActiveFilters" class="flex flex-wrap items-center gap-2 mb-4">
            <button v-if="filters.category" @click="removeFilter('category')"
              class="inline-flex items-center gap-1 bg-slate-100 text-slate-700 text-xs font-medium px-3 py-1 rounded-full hover:bg-slate-200 transition-colors">
              {{ categoryName }}
      ...
      <div v-if="bundles && bundles.length" class="mt-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div v-for="bundle in bundles" :key="bundle.id"
            class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 flex flex-col gap-4">
            <div class="flex items-start justify-between gap-4">
              ...
              <span class="text-xl font-bold text-slate-800 whitespace-nowrap">€{{ bundle.price }}</span>
            </div>
            <ul class="space-y-1">
            ...
            <form :action="route('cart.add')" method="POST" class="mt-auto">
              ...
              <button type="submit"
                class="w-full bg-slate-800 text-white rounded-lg py-2.5 text-sm font-semibold hover:bg-slate-900 transition-colors cursor-pointer">
                Add Bundle to Cart
              </button>
            </form>
          </div>
