<template>
  <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
    ...
    <form @submit.prevent="submit" class="space-y-4">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">{{ t('booking.days') }}</label>
        <div class="flex items-center gap-3">
          <button type="button" @click="form.quantity = Math.max(1, form.quantity - 1)"
            class="w-9 h-9 rounded-full border border-gray-200 flex items-center justify-center text-gray-600 hover:border-slate-500 hover:text-slate-700 transition-colors cursor-pointer">−</button>
          <span class="w-8 text-center font-semibold text-gray-900">{{ form.quantity }}</span>
          <button type="button" @click="form.quantity = Math.min(90, form.quantity + 1)"
            class="w-9 h-9 rounded-full border border-gray-200 flex items-center justify-center text-gray-600 hover:border-slate-500 hover:text-slate-700 transition-colors cursor-pointer">+</button>
          <span class="text-sm text-gray-500">{{ t('booking.days') }}</span>
        </div>
      </div>

      <div class="border-t border-gray-100 pt-4 space-y-1">
        <div class="flex justify-between text-sm text-gray-600">
          <span>{{ formatPrice(activity.price) }} × {{ form.quantity }} {{ t('booking.days') }}</span>
