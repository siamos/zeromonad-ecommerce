  ...
  <Layout>
    ...
    <div class="max-w-3xl mx-auto px-4 py-10">
      ...
      <div v-if="!['cancelled','refunded'].includes(order.status)" class="bg-white rounded-2xl shadow-sm border border-stone-100 p-6 mb-6">
        <h2 class="font-semibold text-gray-900 mb-5 text-sm">Booking Progress</h2>
        <div class="flex items-center">
          <template v-for="(step, i) in timelineSteps" :key="step.key">
            <div class="flex flex-col items-center flex-1">
              <div :class="[
                'w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors',
                stepDone(step.key) ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-400'
              ]">{{ i + 1 }}</div>
              <span class="text-xs mt-1.5 text-center leading-tight"
                :class="stepDone(step.key) ? 'text-emerald-600 font-medium' : 'text-gray-400'">
                {{ step.label }}
              </span>
            </div>
            <div v-if="i < timelineSteps.length - 1"
              :class="['flex-1 h-0.5 mb-5', stepDone(timelineSteps[i + 1].key) ? 'bg-emerald-600' : 'bg-gray-200']" />
          </template>
        </div>
      </div>
      <div v-else class="bg-red-50 border border-red-100 rounded-2xl p-4 mb-6 text-sm text-red-700 font-medium">
        This booking was {{ order.status }}.
      </div>
          ...
          <div v-for="item in order.items" :key="item.id"
            ...
            <div>
              ...
              <a v-if="ticketForItem(item.id)"
                ...
                class="inline-flex items-center gap-1 text-xs text-emerald-600 hover:text-emerald-800 mt-1.5 underline">
          ...
          <form @submit.prevent="submitReturn">
            ...
            <div class="mb-6">
              ...
              <textarea v-model="returnForm.details" rows="3"
                class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none"
            ...
            <div class="flex gap-3">
              ...
              <button type="submit" :disabled="returnForm.processing"
                class="flex-1 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700 transition disabled:opacity-60">
                Submit Request
              </button>
            </div>
          </form>
