<template>
  <Layout>
    ...
    <div class="max-w-4xl mx-auto px-4 py-10">
        ...
        <div class="md:col-span-1 space-y-4">
          <div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 text-center">
            ...
            <button @click="editingProfile = !editingProfile"
              class="mt-3 text-sm text-emerald-600 hover:underline block mx-auto">
          ...
          <div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-4">
            <div class="flex items-center justify-between mb-3">
              ...
              <button @click="addingAddress = !addingAddress" class="text-xs text-emerald-600 hover:underline">
            ...
            <form v-if="addingAddress" @submit.prevent="submitAddress" class="space-y-2 mb-3">
              <input v-model="addressForm.label" type="text" :placeholder="t('account.address_label')"
                class="w-full rounded-lg border border-stone-200 px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500" />
              ...
              <button type="submit" :disabled="addressForm.processing"
                class="w-full bg-emerald-600 text-white text-xs font-medium py-1.5 rounded-lg hover:bg-emerald-700 disabled:opacity-50 transition-colors">
            ...
            <div v-if="addresses?.length" class="space-y-2">
              <div v-for="addr in addresses" :key="addr.id" class="flex justify-between items-start text-xs text-gray-600">
                <div>
                  ...
                  <span v-if="addr.is_default" class="ml-1 text-emerald-600">({{ t('account.default') }})</span>
          ...
          <div v-if="editingProfile" class="bg-white rounded-2xl border border-stone-100 shadow-sm p-6">
            ...
            <form @submit.prevent="submitProfile" class="space-y-3">
              <div>
                <input v-model="profileForm.name" type="text" :placeholder="t('account.name')"
                  class="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              ...
              <div class="pt-2 border-t border-stone-100">
                ...
                <div>
                  <input v-model="profileForm.current_password" type="password" :placeholder="t('account.current_password')"
                    class="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm mb-2 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
              ...
              <button type="submit" :disabled="profileForm.processing"
                class="w-full bg-emerald-600 text-white text-sm font-medium py-2 rounded-lg hover:bg-emerald-700 disabled:opacity-50 transition-colors">
      ...
      <!-- Saved Items -->
      <div v-if="$page.props.saved_items?.length" class="mt-8 bg-white rounded-2xl border border-stone-100 shadow-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-6">
          <div v-for="saved in $page.props.saved_items" :key="saved.id"
            class="flex gap-3 border border-stone-100 rounded-xl p-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-sm font-bold text-emerald-600 mt-0.5">{{ formatPrice(saved.saveable?.price ?? 0) }}</div>
      ...
      <!-- Referral Program -->
      <div v-if="user?.referral_code" class="mt-8 bg-white rounded-2xl border border-stone-100 shadow-sm p-6">
        ...
        <div class="flex items-center gap-2">
          <input
            :value="referralUrl"
            readonly
            class="flex-1 rounded-lg border border-stone-200 px-3 py-2 text-sm text-gray-700 bg-gray-50 focus:outline-none"
          />
          <button
            @click="copyReferralCode"
            class="shrink-0 bg-emerald-600 text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-emerald-700 transition-colors"
          >
            {{ copied ? 'Copied!' : 'Copy' }}
          </button>
        </div>
