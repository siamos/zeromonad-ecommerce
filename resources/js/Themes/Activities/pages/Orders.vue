<template>
  <Layout>
    <Head :title="t('account.my_bookings')" />

    <div class="max-w-4xl mx-auto px-4 py-10">
      <h1 class="text-3xl font-bold text-gray-900 mb-8">{{ t('account.my_bookings') }}</h1>
        ...
        <div class="md:col-span-1">
          <div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 text-center">
            ...
            <Link :href="route('account.index')"
              class="mt-3 block text-sm text-emerald-600 hover:text-emerald-800 transition-colors">
              ← {{ t('account.overview') }}
            </Link>
            <Link :href="route('logout')" method="post" as="button"
              class="mt-2 block text-sm text-red-500 hover:text-red-700 transition-colors">
              {{ t('account.sign_out') }}
            </Link>
          </div>
        </div>
