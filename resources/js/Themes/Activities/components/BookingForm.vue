<template>
  <div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-6">
    <h3 class="font-bold text-gray-900 text-lg mb-1">Book Your Spot</h3>
    <p v-if="activity.activity_detail?.capacity" class="text-sm text-gray-500 mb-4">
      {{ spotsLeft }} spots remaining
    </p>

    <form @submit.prevent="submit" class="space-y-4">
      <!-- Date (fixed if event_date set, else picker) -->
      ...
      <div>
        ...
        <div class="flex items-center gap-3">
          <button
            ...
            class="w-9 h-9 rounded-full border border-gray-200 flex items-center justify-center text-gray-600 hover:border-emerald-500 hover:text-emerald-600 transition-colors"
          >−</button>
          <span class="w-8 text-center font-semibold text-gray-900">{{ form.quantity }}</span>
          <button
            type="button"
            @click="form.quantity = Math.min(maxQty, form.quantity + 1)"
            class="w-9 h-9 rounded-full border border-gray-200 flex items-center justify-center text-gray-600 hover:border-emerald-500 hover:text-emerald-600 transition-colors"
          >+</button>
        </div>
      </div>
      ...
      <button
        type="submit"
        :disabled="submitting"
        class="w-full bg-emerald-600 text-white py-3 rounded-xl font-semibold hover:bg-emerald-700 transition-colors disabled:opacity-60"
      >
