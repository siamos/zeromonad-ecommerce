<template>
  <div class="min-h-screen flex flex-col bg-gray-50" :class="'palette-' + ($page.props.theme_palette ?? 'indigo')">
    <!-- Navigation -->
        ...
        <div class="flex justify-between items-center h-16">
          ...
          <div class="flex items-center gap-3">
            ...
            <div v-if="$page.props.auth.user" class="relative hidden md:block">
              <button @click="notifOpen = !notifOpen" class="relative text-gray-600 hover:text-indigo-600">
                ...
                <div v-if="notifOpen" class="fixed top-16 right-16 z-40 w-80 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                  ...
                  <div v-if="$page.props.recent_notifications?.length" class="divide-y divide-gray-50 max-h-80 overflow-y-auto">
                    <button v-for="notif in $page.props.recent_notifications" :key="notif.id"
                      ...
                      :class="['w-full text-left px-4 py-3 hover:bg-gray-50 text-sm cursor-pointer', !notif.read_at ? 'bg-indigo-50/40' : '']">
                      <div :class="['font-medium text-gray-900 truncate', !notif.read_at ? 'text-indigo-900' : '']">{{ notif.data?.title }}</div>
