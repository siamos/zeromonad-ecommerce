<template>
  <div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-6">
    ...
    <div v-if="$page.props.flash?.success" class="mb-4 p-3 bg-amber-50 text-amber-700 rounded-xl text-sm">
      {{ $page.props.flash.success }}
    </div>

    <div v-if="!$page.props.auth.user" class="text-sm text-gray-500">
      <Link :href="route('login')" class="text-amber-600 hover:underline">{{ t('review.sign_in') }}</Link> {{ t('review.sign_in_to_leave') }}
    </div>
    ...
    <form v-else @submit.prevent="submit" class="space-y-4">
      ...
      <div>
        ...
        <input v-model="form.title" type="text" maxlength="150"
          class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none" />
      </div>
      ...
      <div>
        ...
        <textarea v-model="form.body" rows="4" required maxlength="2000"
          class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none resize-none" />
      </div>
      ...
      <div>
        ...
        <input type="file" multiple accept="image/*" @change="handleImages"
          class="block w-full text-sm text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-sm file:font-medium file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100 cursor-pointer" />
      ...
      <button type="submit" :disabled="!form.rating || form.processing"
        class="px-5 py-2.5 bg-amber-600 text-white text-sm font-semibold rounded-xl hover:bg-amber-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer">
