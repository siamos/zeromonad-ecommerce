    ...
    <div class="max-w-5xl mx-auto px-4 py-10">
      ...
      <div class="flex items-center gap-0 mb-8 mt-2">
        <div class="flex items-center gap-2">
          <div class="w-7 h-7 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center text-xs font-bold">1</div>
          <span class="text-sm text-amber-600 font-semibold">{{ t('checkout.step_cart') }}</span>
        </div>
        <div class="flex-1 h-px bg-amber-200 mx-3" />
        <div class="flex items-center gap-2">
          <div class="w-7 h-7 rounded-full bg-amber-600 text-white flex items-center justify-center text-xs font-bold">2</div>
          <span class="text-sm text-amber-700 font-semibold">{{ t('checkout.step_details') }}</span>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-6">
              ...
              <div class="space-y-3">
                ...
                >
                  <input type="radio" :value="method.key" v-model="form.payment_method" class="mt-0.5 text-amber-600" />
                  <div>
          ...
          <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 sticky top-6">
              ...
              <div class="space-y-3 mb-4">
                <div v-for="item in cart.items" :key="item.id" class="flex gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-amber-600">{{ item.quantity }} {{ t('booking.nights') }}</div>
                    <div class="text-xs text-gray-500">{{ formatPrice(item.product.price) }} {{ t('activity.per_night') }}</div>
                  </div>
                </div>
              </div>
              ...
              <div v-if="pointsDiscount > 0" class="flex justify-between text-amber-600 text-sm">
              ...
              <div v-if="pointsBalance > 0" class="mt-4 p-3 bg-amber-50 rounded-xl border border-amber-100">
                <div class="flex items-center justify-between mb-2">
                  <span class="text-sm font-medium text-amber-900">{{ t('checkout.use_points') }}</span>
                  <span class="text-xs text-amber-600">{{ pointsBalance }} {{ t('checkout.points_available') }}</span>
                </div>
                <div class="flex items-center gap-2">
                  <input type="number" v-model.number="form.use_points" min="0" :max="maxRedeemable" step="100"
                    class="w-24 border border-amber-200 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" />
              ...
              <button type="submit" :disabled="submitting || !form.payment_method"
                class="w-full mt-5 bg-amber-600 text-white py-3 rounded-xl font-semibold hover:bg-amber-700 transition-colors disabled:opacity-60 cursor-pointer">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div v-for="item in upsells" :key="item.id"
            ...
            <div class="p-3 flex flex-col flex-1">
              ...
              <p class="text-amber-600 font-semibold text-sm mb-3">{{ formatPrice(item.price) }}</p>
              <a :href="route('product.show', item.slug)"
                class="mt-auto w-full py-1.5 text-xs font-semibold rounded-lg bg-amber-50 text-amber-700 hover:bg-amber-100 transition-colors text-center">
