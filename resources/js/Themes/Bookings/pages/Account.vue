        ...
        <div class="md:col-span-1 space-y-4">
          <div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-6 text-center">
            ...
            <button @click="editingProfile = !editingProfile"
              class="mt-3 text-sm text-amber-600 hover:underline block mx-auto">
          ...
          <div class="bg-white rounded-2xl border border-stone-100 shadow-sm p-4">
            <div class="flex items-center justify-between mb-3">
              ...
              <button @click="addingAddress = !addingAddress" class="text-xs text-amber-600 hover:underline">
            ...
            <form v-if="addingAddress" @submit.prevent="submitAddress" class="space-y-2 mb-3">
              <input v-model="addressForm.label" type="text" :placeholder="t('account.address_label')"
                class="w-full rounded-lg border border-stone-200 px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-amber-500" />
              ...
              <button type="submit" :disabled="addressForm.processing"
                class="w-full bg-amber-600 text-white text-xs font-medium py-1.5 rounded-lg hover:bg-amber-700 disabled:opacity-50 transition-colors">
            ...
            <div v-if="addresses?.length" class="space-y-2">
              <div v-for="addr in addresses" :key="addr.id" class="flex justify-between items-start text-xs text-gray-600">
                <div>
                  ...
                  <span v-if="addr.is_default" class="ml-1 text-amber-600">({{ t('account.default') }})</span>
            ...
            <form @submit.prevent="submitProfile" class="space-y-3">
              <div>
                <input v-model="profileForm.name" type="text" :placeholder="t('account.name')"
                  class="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500" />
              ...
              <div class="pt-2 border-t border-stone-100">
                ...
                <div>
                  <input v-model="profileForm.current_password" type="password" :placeholder="t('account.current_password')"
                    class="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm mb-2 focus:outline-none focus:ring-2 focus:ring-amber-500" />
              ...
              <button type="submit" :disabled="profileForm.processing"
                class="w-full bg-amber-600 text-white text-sm font-medium py-2 rounded-lg hover:bg-amber-700 disabled:opacity-50 transition-colors">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-6">
          <div v-for="saved in $page.props.saved_items" :key="saved.id"
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-sm font-bold text-amber-600 mt-0.5">{{ formatPrice(saved.saveable?.price ?? 0) }}</div>
      ...
      <div v-if="user?.referral_code" class="mt-8 bg-white rounded-2xl border border-stone-100 shadow-sm p-6">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="shrink-0 bg-amber-600 text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-amber-700 transition-colors"
          >
            {{ copied ? 'Copied!' : 'Copy' }}
          </button>
        </div>
