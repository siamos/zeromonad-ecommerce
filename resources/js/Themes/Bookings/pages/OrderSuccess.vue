<template>
  <Layout>
    ...
    <OrderSuccessModal
      ...
      view-orders-label="View My Stays"
      shop-label="Browse More Stays"
    />
    <div class="max-w-2xl mx-auto px-4 py-16 text-center">
      ...
      <!-- Guest upsell -->
      <div v-if="isGuest" class="bg-amber-50 border border-amber-100 rounded-2xl p-6 mb-8 text-left">
        <h3 class="font-semibold text-amber-900 mb-1">{{ t('checkout.guest_upsell_title') }}</h3>
        <p class="text-sm text-amber-700 mb-4">{{ t('checkout.guest_upsell_desc') }}</p>
        <Link :href="route('register')"
          class="inline-block px-5 py-2 bg-amber-600 text-white rounded-lg text-sm font-semibold hover:bg-amber-700 transition-colors">
