<template>
  <Layout>
    ...
    <div class="max-w-5xl mx-auto px-4 py-10">
      ...
      <p class="text-gray-500 mb-10">{{ t('blog.subtitle_bookings') }}</p>
        ...
        <article v-for="post in posts.data" :key="post.id"
          ...
          <div class="p-5">
            ...
            <h2 class="font-semibold text-gray-900 mb-2 line-clamp-2">
              <Link :href="route('blog.show', post.slug)" class="hover:text-amber-600 transition-colors">
                {{ post.title }}
              </Link>
            </h2>
            <p class="text-sm text-gray-500 line-clamp-3">{{ post.excerpt }}</p>
            <Link :href="route('blog.show', post.slug)"
              class="mt-4 inline-block text-sm text-amber-600 font-medium hover:underline">
              {{ t('blog.read_more') }}
            </Link>
          </div>
        </article>
